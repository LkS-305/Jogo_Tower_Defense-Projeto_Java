...
public class Game extends Canvas implements Runnable, KeyListener {
    ...
    private HUD hud;
    private boolean rotLeft, rotRight;
    private static final double ROT_SPEED = Math.toRadians(180) / 60.0; // ajuste livre
    ...
    private void tick() {
        ...
        if (rotLeft ^ rotRight) {
            double dir = rotRight ? +1 : -1;
            tower.rotateBy(dir * ROT_SPEED);
        }
    ...
    private void render() {
        ...
        if (gameState == STATE.MENU) {
            ...
            drawCenteredText(g, "Use W, A, S, D , Q , E para girar a torre", 340);
        } 
    ...
    public void keyPressed(KeyEvent e) {
        ...
        if (key == KeyEvent.VK_LEFT) rotLeft = true;
        if (key == KeyEvent.VK_RIGHT) rotRight = true;
        //para atiar manualmente usando espaço
    ...
    public void keyReleased(KeyEvent e) {
        ...
        if (key == KeyEvent.VK_LEFT) rotLeft = false;
        if (key == KeyEvent.VK_RIGHT) rotRight = false;
    }
